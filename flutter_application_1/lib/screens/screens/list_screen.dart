...
class ListScreen extends StatelessWidget {
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () async {
              await _printDailyVisits(context);
            },
          ),
        ],
      ),
  ...
  Future<void> _printDailyVisits(BuildContext context) async {
    try {
      final pdf = pw.Document();

      // Generar contenido del PDF
      final visits = await _dbService.getVisits();
      final groupedByDate = _groupVisitsByDate(visits);
      ...
      // Imprimir PDF usando el paquete `printing`
      await Printing.layoutPdf(onLayout: (format) async => pdf.save());

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('PDF enviado a la impresora'),
        ),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
